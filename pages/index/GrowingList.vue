<template>
  <uni-list class="growingList" :border="false">
    <view class="growingItem" v-for="(growing, growingIndex) in growingList" :key="growing.date">
 ...
 * 列表数据
 */
const growingList = reactive<Zhuan.Growing[]>([])
 ...
 */
function thumbnailPhoto(photo: string) {
 ...
 */
function previewPhoto(growingIndex: number, photoIndex: number) {
  uni.previewImage({
    current: photoIndex,
 ...
 * @param {Number} growingIndex 成长记录索引
 */
function handleViewGrowing(growingIndex: number) {
 ...
 * @param {Number} growingIndex 成长记录索引
 */
function handleEditGrowing(growingIndex: number) {
