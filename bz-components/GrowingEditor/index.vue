<template>
  <view class="page fullPage growingEditor">
    ...
    <view class="baseInfo">
      <view class="item">
        <view>身高：</view>
        <uni-easyinput class="input" type="digit" :clearable="false" v-model="form.height" />
        <view class="unit">cm</view>
      </view>
      <view class="item">
        <view>体重：</view>
        <uni-easyinput class="input" type="digit" :clearable="false" v-model="form.weight" />
...
// 移除照片
function handleDeletePhoto({ tempFile }: any) {
  console.log(tempFile)
  abandonPhoto.add([tempFile.fileID])
}

// 上传照片成功
function handleUploadPhotosSuccess({ tempFiles }: any) {
  /*
   * 加入弃用列表。成长记录提交成功后需要从弃用列表移除。
   * 这样处理的好处是，如果成长记录提交失败，可以将照片弃用，节省服务器空间。
   */
  abandonPhoto.add(tempFiles.map((item: any) => item.fileID))
}
